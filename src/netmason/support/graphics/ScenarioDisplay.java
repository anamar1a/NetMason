 ...
 */
public class ScenarioDisplay implements Steppable {
	...
	protected static UndirectedSparseGraph<Xnode, Xedge> graph;
	...
	public ScenarioDisplay(NetMasonWithUI withUI) {
	...
	private static JPanel getGraphPanel() {
		...
		Transformer vertexLabeller = new Transformer<Xnode, String>() {
		...
		Transformer vertexPainter = new Transformer<Xnode, Paint>() {
		...
		Transformer vertexDrawTransformer = new Transformer<Xnode, Paint>() {
		...
		Transformer edgeStrokeTransformer = new Transformer<Xedge, Stroke>() {
	...
	public void recreateGraph() {
		...
		graph = new UndirectedSparseGraph<Xnode, Xedge>();

		for (Xnode a : sim.masterGraph.getVertices()) {
			...
			if (!(a.isType("task"))) {
				...
				for (Xnode z : sim.masterGraph.getNeighbors(a)) {

					if ((!(graph.containsVertex(z))) && (!(z.isType("task")))) {
						graph.addVertex(z);
					}
					
					if ((!(graph.isNeighbor(a, z)) && (!(z.isType("task"))))) {
						graph.addEdge(new Xedge(), a, z);
					}
				}
			}
		}
