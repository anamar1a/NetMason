 ...
 */
public class CommunicationsDisplay implements Steppable {
	...
	protected static Graph<Operative, Xedge> graph;
	...
	public CommunicationsDisplay(NetMasonWithUI withUI) {
	...
	public void recreateGraph() {
		...
		ArrayList<Operative> agentList = sim.nodeTable.get(Operative.class);
		graph = new SparseGraph<Operative, Xedge>();

		for (Operative a : agentList) {

			graph.addVertex(a);

			for (Xedge e : a.beliefs.getEdges()) {

				Xnode a1 = a.beliefs.getEndpoints(e).getFirst();
				Xnode a2 = a.beliefs.getEndpoints(e).getSecond();

				if ((a1.isType("agent")) && (a2.isType("agent"))) {

					if (!graph.containsVertex((Operative) a1)) {
						graph.addVertex((Operative) a1);
					}

					if (!graph.containsVertex((Operative) a2)) {
						graph.addVertex((Operative) a2);
					}

					if (!graph.isNeighbor((Operative) a1, (Operative) a2)) {
						Xedge tempEdge = new Xedge();
						tempEdge.properties.put("timesKnown", new Double(1).toString());
						graph.addEdge(tempEdge, (Operative) a1, (Operative) a2, EdgeType.UNDIRECTED);
					} else {

						Xedge tempEdge = graph.findEdge((Operative) a1, (Operative) a2);
						Double weight = new Double(tempEdge.properties.get("timesKnown"));
						weight = weight + 0.25;
						tempEdge.properties.put("timesKnown", new Double(weight).toString());
					}
