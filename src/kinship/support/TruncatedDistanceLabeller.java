...
public class TruncatedDistanceLabeller<V, E> {
	 ...
	 * MutableInteger
	 */
	public TruncatedDistanceLabeller(KinshipModel myWorld) {
		mPredecessorMap = new HashMap<V, HashSet<V>>();
		this.kinshipHorizon = 2 * myWorld.parameterMap.get("splitRadius").intValue();
	}
	...
	public void labelDistances(Graph<V, E> graph, Set<V> rootSet) {
		...
		while (true) {
			...
			if ((newList.size() == 0) || (distance > this.kinshipHorizon))
				break;
	 ...
	 * keeps track of the predecessors of each node traveresed as well as the
